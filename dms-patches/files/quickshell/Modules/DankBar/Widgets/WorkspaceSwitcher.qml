...
Item {
    ...
    function getNiriWorkspaces() {
        ...
        return capWorkspacesToVisible(workspaces);
    }

    // piBrick: this device's screen is too narrow for an unbounded workspace
    // pill count - keep at most 2 visible, centered on whichever is active.
    function capWorkspacesToVisible(list) {
        const maxVisible = 2;
        if (list.length <= maxVisible)
            return list;
        const activeIdx = list.findIndex(ws => ws.is_active);
        if (activeIdx === -1)
            return list.slice(0, maxVisible);
        const start = Math.max(0, Math.min(activeIdx, list.length - maxVisible));
        return list.slice(start, start + maxVisible);
    }

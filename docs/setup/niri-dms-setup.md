...
DMS auto-generates its own binding for these two keys in `~/.config/niri/dms/binds.kdl` (`dms ipc
call audio increment/decrement 3`); that file is regenerated by DMS and must not be hand-edited. Niri
merges `binds { }` blocks from all included files, with later-appearing bindings for the same key
overriding earlier ones — confirmed via niri's own window-rule docs, which document the same override
behavior for `window-rule` blocks. Since the user's own `config.kdl` `include`s `dms/binds.kdl` before
defining its own `binds { }` block, redefining just these two keys there is enough; the DMS-generated
file is left untouched. Verified live: `niri msg action load-config-file` reported no errors, `niri
validate` confirmed `config is valid`, and the user confirmed both directions work correctly on-device.

## Side wheel doubles as volume control (held with Alt)

The keyboard module's side rotary encoder reports as a standard scroll wheel (`REL_WHEEL`, see
`docs/hardware/overview.md`'s keyboard-module section) and, unmodified, is niri's native input for
scrollable-tiling column/workspace navigation on this device — the same role a laptop's two-finger
trackpad scroll plays. So rather than repurpose it outright (as was done for the two side buttons
above), volume is bound to the same wheel *held with Alt*, leaving plain wheel ticks free for
navigation:

- `Alt+WheelScrollUp` → `dms ipc call audio increment 3`
- `Alt+WheelScrollDown` → `dms ipc call audio decrement 3`

`Alt` was confirmed free to use as a plain modifier here because `Mod` (used throughout the rest of
`binds { }`) resolves to `Super` on this device — niri only aliases `Mod` to `Alt` when running nested
as a winit window, not on a TTY session like this one. The exact same `dms ipc call audio
increment/decrement 3` command DMS itself uses for the `XF86AudioRaiseVolume`/`XF86AudioLowerVolume`
keys (see above) was reused here rather than calling `wpctl` directly, so the volume OSD and bar
slider stay in sync. Verified live the same way as the button remap above: `niri msg action
load-config-file` / `niri validate` both clean, user confirmed both directions work on-device.
